...
public class EventConsumer implements CommunityConstant {
    ...
    //消费 生成长图
    @KafkaListener(topics = TOPIC_SHARE)//kafka监听的topic通道，从中获取消息信息
    public void generateImage(ConsumerRecord record) {
        ...
        //获取生成长图需要的信息
        String htmlUrl = (String) event.getData().get("htmlUrl");//需要生成长图的网站地址
        String fileName = (String) event.getData().get("fileName");//生成的长图的图片名称
        String suffix = (String) event.getData().get("suffix");//图片的后缀类型
        ...
        //启动定时器 监控图片是否生成 一旦生成就立即将图片上传到七牛云
        UploadTask uploadTask = new UploadTask(fileName,suffix);//创建任务
        Future future = taskScheduler.scheduleAtFixedRate(uploadTask, 500);//线程每半秒就检查一次任务
        ...
        public void run() {
            ...
            if (file.exists()) {
                ...
                try {
                    ...
                    //这里应该将上传的文件类型后缀进行切割  因为这里传入的suffix为.png  而我们需要的是png
                    /*Response response = manager.put(
                            path, fileName, uploadToken, null,
                            "image/" + suffix, false);*/
                    // 处理响应结果
